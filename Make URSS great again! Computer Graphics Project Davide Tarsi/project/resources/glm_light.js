...
//        model.normals[3 * i + 2] -= cz;
	...
	*/
	...
	function Triangle() {
	  ...
	  this.nt = 0;			/* number of polygon vertices */
	...
	function SecondPass(model, data, mesh) {
	  ...
	  mesh.nnormals = model.numnormals;
	  mesh.ntexcoord = model.numtexcoords;
    ...
    //numvertices = numnormals = numtexcoords = 1;
     ...
     mesh.normal = new Array(mesh.nnormals+1);	  
	 for (var h = 0; h<mesh.nnormals+1; h++) 
		mesh.normal[h] = new nnr();
	
	 mesh.texcoord = new Array(mesh.ntexcoord+1);	  
	 for (var h = 0; h<mesh.ntexcoord+1; h++) 
		mesh.texcoord[h] = new ntext();
...
//         console.log(1,numvertices);
		...
		case 'vn':               /* v, vn, vt */
			...
			mesh.normal[numnormals].x=parseFloat(buf[1]); 
			mesh.normal[numnormals].y=parseFloat(buf[2]); 
			mesh.normal[numnormals].z=parseFloat(buf[3]); 
		...
		case 'vt':               /* v, vn, vt */
			...
			mesh.texcoord[numtexcoords].x=parseFloat(buf[1]); 
			mesh.texcoord[numtexcoords].y=parseFloat(buf[2]); 
		...
		case 'f':               /* face */
			...
			//formati: v1/vt1/vn1 or v1//vn1
			//buf[1]= prima doppietta/tripletta, buf[2] la seconda, buf[3] la terza -> separatore " "
			//f1 considera la prima doppietta/tripletta restituisce stringa senza separatore ='/'
			//es. 2/4/3 -> 2,4,3   in cui f1[0]=2 f1[1]=4 f1[2]=3
			//es. 2//3 -> 2,,3	   in cui f1[0]=2 f1[1]=false f1[2]=3
			//in ogni caso la posizione [2] contiene informazione indice della normale
			//la posizione [1] se true allora leggo informazioni texture se false vuol dire che sto leggendo v1//vn1
			
			//prima doppietta/tripletta
		    model.triangles[numtriangles].vindices[0] = parseFloat(f1[0]);
			if (f1[1]){
				model.triangles[numtriangles].tindices[0] = parseFloat(f1[1]);
				//se avro coordinata texture per la prima tripletta allora suppongo che ho questa informazione anche per le altre due triplette
				model.triangles[numtriangles].nt = 3; 
			}
			model.triangles[numtriangles].nindices[0] = parseFloat(f1[2]);
			
			//seconda doppietta/tripletta
			model.triangles[numtriangles].vindices[1] = parseFloat(f2[0]);
		    if (f2[1])model.triangles[numtriangles].tindices[1] = parseFloat(f2[1]);
			model.triangles[numtriangles].nindices[1] = parseFloat(f2[2]);
			
			//terza doppietta/tripletta
		    model.triangles[numtriangles].vindices[2] = parseFloat(f3[0]);
		    if (f3[1])model.triangles[numtriangles].tindices[2] = parseFloat(f3[1]);
			model.triangles[numtriangles].nindices[2] = parseFloat(f3[2]);
			
			
			model.triangles[numtriangles].nv = 3; //ho sempre info sui vertici
			...
			for (var jj=0; jj<mesh.face[numtriangles].n_v_e; jj++)
			{
				...
				mesh.face[numtriangles].normal[jj]=model.triangles[numtriangles-1].nindices[jj];
			}
			
			mesh.face[numtriangles].n_t=model.triangles[numtriangles-1].nt;
			for (var jj=0; jj<mesh.face[numtriangles].n_t; jj++)
			{
				mesh.face[numtriangles].texcoord[jj]=model.triangles[numtriangles-1].tindices[jj];
			}
